...
class StockData:
    ...
    def get_recent_index_data(self, index):
        ...
        recent_data['Change'] = recent_data['Change'].apply(lambda x: '+' + str(round(x * 100, 2)) + '%' if x > 0 else str(round(x * 100, 2)) + '%')
        # 증감액에 부호 추가하고 소수점 둘째자리까지 표시
        recent_data['ChangeAmount'] = recent_data['ChangeAmount'].apply(lambda x: '+' + str(round(x, 2)) if x > 0 else str(round(x, 2)))
        return recent_data
